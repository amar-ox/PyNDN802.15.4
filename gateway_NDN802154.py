"""
    ...
    It allows a gateway to communicate with Named Data Networking devices over IEEE802.15.4. 
"""
...
eCP = Name("/farm1/cows").wireEncode().buf()[2:]
lCP = len(eCP)
buffInterest = bytearray(100)
buffData = bytearray(100)

p ='/dev/serial/by-id/usb-FTDI_FT232R_USB_UART_A4008SYk-if00-port0'
ser = serial.Serial(port=p, baudrate=38400)
ieee = IEEE802154(ser)
...
def onInterest(prefix, interest, face, interestFilterId, filter):
    ...
    enc = interest.wireEncode().buf()
    buffInterest[0] = enc[0]
    buffInterest[1] = enc[1] - lCP
    buffInterest[2] = enc[2]
    buffInterest[3] = enc[3] - lCP
    buffInterest[4:] = enc[lCP+4:]

    e.acquire() 
    ieee.send('tx', dest_addr=b'\xff\xff', options=b'\x01', data=buffInterest)
    e.release()
...
def main():
    ...
    pibImpl = PibMemory()
    keyChain = KeyChain(pibImpl, TpmBackEndMemory(), SelfVerifyPolicyManager(pibImpl))
    # This puts the public key in the pibImpl used by the SelfVerifyPolicyManager.
    keyChain.importSafeBag( SafeBag(Name("/testname/KEY/123"), Blob(DEFAULT_RSA_PRIVATE_KEY_DER, False), Blob(DEFAULT_RSA_PUBLIC_KEY_DER, False)) )

    backboneFace.setCommandSigningInfo(keyChain, keyChain.getDefaultCertificateName())
    ...
    while 1:
        try:
            ...
            if frame is not None:
                if frame['rf_data'][0] == b'\x06' or frame['rf_data'][0] == b'\x05':	#if Data or Interest
